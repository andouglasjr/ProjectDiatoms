...
data_transforms = {
    ...
    'test': transforms.Compose([
        ...
        #transforms.Grayscale(1),
        transforms.ToTensor()
        #transforms.Normalize([0.496, 0.496, 0.496], [0.07, 0.07, 0.07])
        #transforms.Normalize([0.496], [0.07])
    ]),
}
...
data = DataUtils(list_of_name_folders, data_dir, data_transforms, net_name = model_name)
...
model_ft = ModelClass(model_name=model_name, folder_names = list_of_name_folders)
model = model_ft.get_model()
best_model = model_ft.train_model(model, dataloaders, params, dataset_size, data)
model_ft.save_model(best_model, 'results/Resnet50_no_normalization.pt')
